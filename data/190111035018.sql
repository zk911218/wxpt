...
CREATE TABLE `wx_pt_login` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=30 DEFAULT CHARSET=utf8 COMMENT='登陆记录';
...
-- ----------------------------
--  Table structure for `wx_pt_token`
-- ----------------------------
DROP TABLE IF EXISTS `wx_pt_token`;
CREATE TABLE `wx_pt_token` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `access_token` varchar(512) NOT NULL,
  `token_time` timestamp NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT 'Token的申请时间，自动刷新',
  `expires_in` int(11) DEFAULT NULL COMMENT 'Token的有效期',
  `token_type` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='存储从微信请求的Token';
