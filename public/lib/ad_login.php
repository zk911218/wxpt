<?php
    ...
    if ($uid != null && $uid != "" && $upwd != null && $upwd != "") {
        $sql   =   "SELECT * FROM wx_pt_ad WHERE ad_id = '$uid'";
        ...
        if (password_verify ( $upwd , $yh_pwd )) {
            ...
            $_SESSION['adUser'] = $yh;
            $id = $yh["id"];
			$sql2 = "INSERT INTO wx_pt_login(`ip`, `uid`,`type`,`state`) VALUES ('$ip','$id','ad','success')";
            mysqli_query($link,$sql2);
            echo "success";
        } else {
            // 密码错误
            $id = $yh["id"];
            $sql2 = "INSERT INTO wx_pt_login(`ip`, `uid`,`type`,`state`) VALUES ('$ip','$id','ad','password_error')";
            mysqli_query($link,$sql2);
            echo "error";
        }
    } else {
        // 数据错误，记录日志
        $id = $yh["id"];
        $sql2 = "INSERT INTO wx_pt_login(`ip`, `uid`,`type`,`state`) VALUES ('$ip','$id','ad','login_illegal')";
        mysqli_query($link,$sql2);
        echo "error";
    }
